...
OFFICIAL_EICAR_URL = "https://secure.eicar.org/eicar.com.txt"
...
def record_step(
    ...
    msg = f"[{status}] {technique} | {step} | {detail}"
    if path:
        msg += f" | {path}"
...
def create_eicar_file(workdir: Path, report: Dict[str, Any]) -> Path:
    """
    T1059 - Command and Scripting Interpreter
    ...
    eicar_path = workdir / "eicar_test.com.txt"

    try:
        eicar_path.write_bytes(EICAR_BYTES)

        record_step(
            report=report,
            technique="T1059",
            step="create_eicar_file",
            status="OK",
...
def read_only_access(eicar_path: Path, report: Dict[str, Any]) -> None:
    """
    T1204 - User Execution
...
def optional_official_download(
    workdir: Path,
    report: Dict[str, Any],
    timeout: int = 15,
) -> Path:
    ...
    try:
        ...
        if content == EICAR_BYTES:
            status = "OK"
            detail = f"Downloaded EICAR from official HTTPS URL: {OFFICIAL_EICAR_URL}"
        else:
            status = "UNEXPECTED_CONTENT"
            detail = (
...
def create_non_encrypted_zip(
    ...
    zip_path = workdir / "eicar_test_plain_not_encrypted.zip"

    try:
        if not eicar_path.exists():
            raise FileNotFoundError(
                f"Source EICAR file not found. It may have been quarantined: {eicar_path}"
            )
...
def main() -> int:
    ...
    eicar_path = create_eicar_file(
        workdir=workdir,
        report=report,
    )

    read_only_access(
        eicar_path=eicar_path,
        report=report,
    )

    if args.download_official:
        optional_official_download(
            workdir=workdir,
            report=report,
        )
    else:
